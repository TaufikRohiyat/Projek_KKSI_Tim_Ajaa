...
<center><h3>DATA PEMINJAMAN BARANG</h3></center>
<?php 
...
<?php
	$no=1;
	$sqlCount = "select count(tanggal_pinjam) from pinjam where status='Y'";
	...
	$sql_limit = "select * from pinjam where status='Y' order by tanggal_pinjam limit $mulai_dari, $limit";
...
?>
	...
	<table class="table table-dark">
		<thead>
			<a href="data_pinjam_semua.php" class="btn btn-warning" type="button">Lihat semua data peminjaman</a>
			<tr class="success">
				<td>No</td>
				<td>Nama Siswa</td>
				<td>Nama Barang</td>
...
?>
	<tr class="success">
		...
		<td><a href= "kembalikan.php?id=<?php echo $data[0];?>" class="btn btn-success" type="button">Kembalikan</a></td>
	</tr>
...
<?php
	$banyakHalaman = ceil($banyakData / $limit);
		...
		for($i = 1; $i <= $banyakHalaman; $i++){
			if($page != $i){
				echo '<li class="page-item"><a class="page-link" href="data_pinjam.php?page='.$i.'">Page '.$i.'</a></li> ';
			}else{
